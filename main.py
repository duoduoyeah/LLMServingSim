...
def main():
    ...
    parser.add_argument(
        "--model_name", type=str, help="Name of the model", default="gpt3-6.7b"
    )
    parser.add_argument(
        "--hardware", type=str, help="type of a hardware (e.g. A100)", default="RTX3090"
    )
    parser.add_argument("--npu_num", type=int, help="# of NPUs", default=16)
    parser.add_argument(
        "--max_batch", type=int, help="maximum size of the batch", default=0
    )
    parser.add_argument(
        "--npu_group", type=int, help="npu_group to control parallelism", default=1
    )
    parser.add_argument("--npu_mem", type=int, help="npu memory in GB", default=40)
    parser.add_argument(
        "--local_bw",
        type=int,
        help="bandwidth of local (device) memory in GB",
        default=1024,
    )
    parser.add_argument(
        "--remote_bw",
        type=int,
        help="bandwidth of remote (host) memory in GB",
        default=512,
    )
    parser.add_argument(
        "--link_bw", type=int, help="bandwidth of link in GB", default=256
    )
    parser.add_argument(
        "--fp", type=int, help="size of floating point in bit", default=16
    )
    parser.add_argument(
        "--block_size", type=int, help="kv cache block size unit of tokens", default=8
    )
    ...
    parser.add_argument(
        "--gen", action="store_false", default=True, help="skip initiation phase"
    )
    parser.add_argument(
        "--req_num", type=int, help="number of requests to use", default=100
    )
    parser.add_argument(
        "--log_interval",
        type=float,
        help="interval to log throughput (sec)",
        default=0.5,
    )
    parser.add_argument(
        "--verbose", action="store_true", default=False, help="make verbose"
    )
    ...
    max_batch = (
        args.max_batch if args.max_batch != 0 else float("inf")
    )  # 0 means infinite batch size
    npu_group = (
        args.npu_group
    )  # configure this to control parallelism      *if npu_group == 1: tensor parallelism, npu_num == npu_group: pipeline parallelism
    npu_mem = (
        args.npu_mem
    )  # npu local mem (hbm) in GB     *if pim pool mode, it is size of pim and kv cache is in pim
    ...
    memory = setRemoteBandwidth(
        astra_sim + "/inputs/remote_memory/analytical/per_npu_memory_expansion.json",
        remote_bw,
    )
    binary = (
    ...
    scheduler = Scheduler(
        model, max_batch, npu_num, npu_group, npu_mem, fp, block_size, req_num, verbose
    )
    ...
    while True:
        ...
        if current > last_log + INTERVAL:
            ...
            last_log += INTERVAL
            print(
                f"[{last_log/FREQ}s] Avg Throughput: propmt: {prompt_th*RATIO}, generation: {gen_th*RATIO}"
            )
            prompt_th = 0
            gen_th = 0
