...
class MemoryModel:
    ...
    def getBatchKV(self, batch_req, batch_len):
        ...
        for i in range(batch_len):
            num_blocks = (
                batch_req[i].input // self.block_size + 1
            )  # it includes kv_cache that will be generated in current iteration
    ...
    def getBlockKV(self, batch_req, batch_len):
        ...
        for i in range(batch_len):
            if batch_req[i].evict or batch_req[i].isInit:
                ...
                ):  # difference of the block is maximum one block
...
def calculateSizes(model, layer_name, length, init=False, fp=2):
    ...
    if layer_name == "vocab_embedding":
        ...
        if init:
            ...
            input_size = (
                (2 * length + 1) * n_embd * fp
            )  # Q (1) + K (kv len) + V (kv len)
            weight_size = 0
