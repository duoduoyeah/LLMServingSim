    ...
    for i in range(npus_per_group):
        ...
        if i in attn_npu1 and attn_npu1[i] > int(qkv2[1]) // npus_per_group:
            b.append(
                [
                    "attn_overlap_1",
                    f"{attn_npu1[i]-int(qkv2[1])//npus_per_group}",
                    "LOCAL",
                    "0",
                    "REMOTE",
                    "0",
    ...
    for i in range(npus_per_group):
        ...
        if i in attn_npu1 and attn_npu1[i] > block2_gemm // npus_per_group:
            block2.append(
                [
                    "attn_overlap_1",
                    f"{attn_npu1[i]-block2_gemm//npus_per_group}",
                    "LOCAL",
                    "0",
                    "REMOTE",
                    "0",
        ...
        if i in attn_npu2 and attn_npu2[i] > block1_gemm // npus_per_group:
            block1.append(
                [
                    "attn_overlap_1",
                    f"{attn_npu2[i]-block1_gemm//npus_per_group}",
                    "LOCAL",
                    "0",
                    "REMOTE",
                    "0",
    ...
    for i in range(npus_per_group):
        ...
        if i in attn_npu2 and attn_npu2[i] > end1_gemm // npus_per_group:
            end1.append(
                [
                    "attn_overlap_1",
                    f"{attn_npu2[i]-end1_gemm//npus_per_group}",
                    "LOCAL",
                    "0",
                    "REMOTE",
                    "0",
...
def dataset_converter(input_file_path, output_file_path):
    ...
    alpaca_data_transformed = pd.DataFrame(
        {"input_toks": alpaca_data["seq_len"], "output_toks": 1, "arrival_time_ns": 0}
    )
