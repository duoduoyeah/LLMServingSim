...
def createNetworkConfig(astra_sim, npu_nums, npu_group, local_bw, link_bw):
    ...
    output_file = (
        astra_sim
        + f"/inputs/network/analytical/fullyconnected_{network_dim}d_{npu_nums}.json"
    )
    ...
    topology_data = {
        ...
        "topologies-per-dim": [
            "FullyConnected" for _ in range(network_dim)
        ],  # Connection type of each dimension (refer to astra-sim paper)
        ...
        "links-count": [
            (npus_per_dim - 1 if npus_per_dim != 1 else 1) if i == 0 else 1
            for i in range(network_dim)
        ],
